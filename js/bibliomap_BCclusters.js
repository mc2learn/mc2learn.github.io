...
function doBCviz() {
  ...
  d3.json(defaultfile, function(data) {
      ...
      if(levels.length==2){
        ...
        //foo='<strong>'+data.ncom[0]+' topics</strong> (Q='+data.Q[0]+') and <strong>'+data.ncom[1]+' subtopics</strong> (Q='+data.Q[1]+')'
        foo='<strong>'+data.ncom[0]+' topics</strong>'
        d3.select("#geninfo").html(foo)
      }
      if(levels.length==1){
        //LIST_DISC=[data.listS]
        maxNodeSize = [ data.TOPmaxNodeSize ]
        maxLinkWeight = [ data.TOPmaxLinkWeight]
        Wrange = [ data.TOPWrange ]
        showthr = [ data.TOPshowthr ]
        forceparam = [ data.TOPforce ]
        DEFpositions = [ data.TOPpositions ]
        d3.select("#Nbc").html(data.Nbc)
        foo='<strong>'+data.ncom[0]+' topics</strong> (Q='+data.Q[0]+')'
        d3.select("#geninfo").html(foo)
      ...
      // ...fixed or dynamic
      // TODO: TEMPORARILY comment out these 3 lines to hide UI for Judi - PS
...
function updateA(){
  ...
  if (keepitem.indexOf("MCAU")>-1){thehtml += '<option value="cA"'+(mylabel=="cA"?'selected':'')+'>the most publishing/cited authors</option>'}
  ...
  */
  ...
  function getclusterInfo( n ) {
      ...
      info += '<div class="stuff" style="text-align:left;font-size:13px;">Total publications:</b> '+ n.size + '<br/></div><br/>'
      info += '<div class="stuff" style="text-align:left;font-size:13px;">'+ n.descr + '<br/></div><br/>'
      ...
      keepitem.forEach(function(elt){
        // most used items
        if(elt.length<3 || elt=="IAEA"){info += itemsTables(n.stuff[elt], n.available[elt], elt, itemfoos[elt][0], itemfoos[elt][1])}
  ...
  d3.json(datafile,
    ...
    if (mylabel == undefined){document.getElementById("mylabel").value='cA'}
    updatelabel();
    ...
    function updatelabel(){
      ...
      if (mylabel==='fIAEA'){nodeArray.forEach(function(n){n.labelb=n.stuff.IAEA[0][0]; });}
      if (mylabel==='sIAEA'){nodeArray.forEach(function(n){n.labelb=mostsign(n.stuff.IAEA); });}
    ...
    function showClusterPanel( node ) {
      ...
      d3.select("#myjump").on('change',function(){
        ...
        document.getElementById("clusterInfo").scrollTop=100+document.getElementById("info_"+foo).getBoundingClientRect().top-document.getElementById("info_MRP").getBoundingClientRect().top
      });
    }
